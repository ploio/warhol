...
docker build -t warhol/release .

echo -e "\033[32;01m[$APP] Make binaries \033[0m"
docker run --rm \
       -v `pwd`:/tmp/ \
       warhol/release \
       gox "${OS_PLATFORM_ARG[@]}" "${OS_ARCH_ARG[@]}" -output="/tmp/warhol_{{.OS}}-{{.Arch}}"

echo -e "\033[32;01m[$APP] Make release \033[0m"
docker run --rm -e GITHUB_TOKEN warhol/release \
...
for BINARY in warhol_*; do
    docker run --rm -e GITHUB_TOKEN -v `pwd`:/go/src/github.com/portefaix/warhol \
       warhol/release github-release upload \
       --user $USERNAME \
       --repo $REPO \
       --tag $VERSION \
       --name $BINARY \
